...
/**
 * where usernum in (?,?,?,?,?,?,?,?,?,?)
 * order by begintime desc limit 200
 */
public class TestThread3 implements Runnable {
    private static final Logger logger = Logger.getLogger(TestThread3.class);
    ...
    public TestThread3(String name, ArrayList<String> numberList, ArrayList<String> tableList){
        this.name = name;
            ...
            try (Connection connection = DriverManager.getConnection(url, user, password)) {
                ...
                while (true) {
                    try {
                        ...
                        String sql = String.format("select * from %s where usernum in ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s') order by begintime desc limit 200",
...
//                            try (ResultSet resultSet = statement.executeQuery(String.format("Select * from (Select row_number() over(order by begintime desc) rn,t.* from tb_evt t where usernum= '%s') where rn>=1 and rn<=200", number))) {
                        ...
                        try (PreparedStatement preparedStatement = connection.prepareStatement("insert into tb_test_record_sql3(thread_name,missionid,query_num,return_time,count_finish_time,count,record_time)values(?,?,?,?,?,?,sysdate)")) {
