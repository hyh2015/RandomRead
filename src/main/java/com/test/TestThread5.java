...
/**
 * count(*) over (partition by order by ) where usernum in (?,?,?,?,?,?,?,?,?,?)
 */
public class TestThread5 implements Runnable {
    private static final Logger logger = Logger.getLogger(TestThread5.class);
    ...
    public TestThread5(String name, ArrayList<String> numberList, ArrayList<String> tableList){
        this.name = name;
            ...
            try (Connection connection = DriverManager.getConnection(url, user, password)) {
                ...
                while (true) {
                    try {
                        ...
                        String sql = String.format("select * from (select count(*) over (partition by usernum) ct, t.* " +
                                        "from %s t where usernum in('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s'))",
                        ...
                        try (PreparedStatement preparedStatement = connection.prepareStatement("insert into tb_test_record_sql5(thread_name,missionid,query_num,return_time,count_finish_time,count,record_time)values(?,?,?,?,?,?,sysdate)")) {
