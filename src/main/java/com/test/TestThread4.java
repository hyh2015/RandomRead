...
/**
 * row_number() over (partition by dt order by begintime) where usernum in(?,?,?,?,?,?,?,?,?,?)
 */
public class TestThread4 implements Runnable {
    private static final Logger logger = Logger.getLogger(TestThread4.class);
    ...
    public TestThread4(String name, ArrayList<String> numberList, ArrayList<String> tableList){
        this.name = name;
            ...
            try (Connection connection = DriverManager.getConnection(url, user, password)) {
                ...
                while (true) {
                    try {
                        ...
                        String sql = String.format("SELECT * FROM (SELECT ROW_NUMBER() OVER(PARTITION BY USERNUM ORDER BY BEGINTIME DESC) RN, t.* FROM %s t " +
                                        "WHERE USERNUM IN ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')) WHERE RN <= 200",
...
//                            try (ResultSet resultSet = statement.executeQuery(String.format("select * from tb_evt WHERE USERNUM = '%s'", number))) {
//                            try (ResultSet resultSet = statement.executeQuery(String.format("Select * from (Select row_number() over(order by begintime desc) rn,t.* from tb_evt t where usernum= '%s') where rn>=1 and rn<=200", number))) {
                        ...
                        try (PreparedStatement preparedStatement = connection.prepareStatement("insert into tb_test_record_sql4(thread_name,missionid,query_num,return_time,count_finish_time,count,record_time)values(?,?,?,?,?,?,sysdate)")) {
